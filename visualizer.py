...
def update(algorithm, swap1=None, swap2=None, display=display): # The function responsible for drawing the sorted array on each iteration
    ...
    for i in range(len(algorithm.array)):
        ...
        height = int((algorithm.array[i] / max(algorithm.array)) * dimensions[1])
        pygame.draw.rect(display, colour, (i * k, dimensions[1] - height, k, height))    
    # time.sleep(0.1)
    check_events()
